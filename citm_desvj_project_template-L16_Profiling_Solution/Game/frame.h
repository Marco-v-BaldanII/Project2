...
public:
	...
	void Render(float dt, int currentHP, int currentAtk, int currentSpeed, int currentPrecision, int currentLuck, int currentMovement, int hpMod = 0, int atkMod = 0, int spdMod = 0, int precMod = 0, int luckMod =0) {
		...
		app->render->DrawText(HPString, (startingPos.x +810+30) + (app->render->camera.x / 3), (startingPos.y + 73+200)  + (app->render->camera.y / 3), 70, 50, false, SDL_Color{ 254, 254, 0, 255 });
		string s = "+ "; s += to_string(hpMod);
		if(hpMod != 0)app->render->DrawText(s, (startingPos.x  + 810) + (app->render->camera.x / 3) + 30, (startingPos.y + 73 + 200) + (app->render->camera.y / 3) - 30, 70, 50, false, SDL_Color{254, 254, 254, 255}, CENTER_ALIGN);
		...
		app->render->DrawText(precision, (startingPos.x + 150+950-75)  + (app->render->camera.x / 3), (startingPos.y + 100+250) + (app->render->camera.y / 3), 30, 50, false, SDL_Color{ 254, 254, 0, 255 });


		app->render->DrawText(luck, (startingPos.x + 95+830-50)  + (app->render->camera.x / 3), (startingPos.y + 127+300)  + (app->render->camera.y / 3), 30, 50, false, SDL_Color{ 254, 254, 0, 255 });
		s = "+ "; s += to_string(luckMod);
		if (luckMod != 0)app->render->DrawText(s, (startingPos.x + 95 + 830)-75 + (app->render->camera.x / 3) + 30, (startingPos.y + 100 + 250) + (app->render->camera.y / 3) - 30, 70, 50, false, SDL_Color{ 254, 254, 254, 255 }, CENTER_ALIGN);

		app->render->DrawText(movement, (startingPos.x + 150+950-75)  + (app->render->camera.x / 3), (startingPos.y + 127+300)  + (app->render->camera.y / 3), 30, 50, false, SDL_Color{ 254, 254, 0, 255 });
