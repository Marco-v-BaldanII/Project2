...
public:
	...
	bool Render() {
		if (active) {
			for (ListItem<Particle*>* pIt = myParticles.start; pIt != NULL; pIt = pIt->next) {
				if (pIt->data != NULL && pIt->data->active) {
					...
					if (!animated) {
						if (flipped) {
							...
							else { app->render->DrawTexture(particle->maintexture, particle->position.x , particle->position.y, &r, false, particle->currentAlpha); }
						}
					}
					else {
